...
app.post("/api/rtp", async (req, res) => {
  try {
    ...
    const payload = { aggregator_merchant_identifier, amount, order_id, request_id, type };
  ...
  try {
    ...
    if (qt !== "STATIC") {
      if (amount === undefined || amount === null || Number(amount) <= 0) {
        return res.status(400).json({ error: "Missing/invalid amount for DYNAMIC QR" });
      }
    }

    const body = { type: qt, aggregator_merchant_identifier, order_id, request_id };
    if (qt === "DYNAMIC") body.amount = amount;
...
app.post("/api/payout", async (req, res) => {
  try {
    const { amount, creditor_iban, request_id } = req.body;

    const a = Number(amount);
    if (!Number.isFinite(a) || a <= 0) {
      return res.status(400).json({ error: "Missing/invalid amount" });
    }
    if (!creditor_iban || !String(creditor_iban).trim()) {
      return res.status(400).json({ error: "Missing creditor_iban" });
    }
    ...
    const payload = {
      ...
      amount: String(amount), // you provided it as string in the example
      creditor_iban: String(creditor_iban).trim(),
      type: "PAYOUT",
    };

    const url = `https://api.getsafepay.com/raastwire/v1/aggregators/${process.env.RAAST_AGGREGATOR_ID}/payout`;
    ...
    res.status(500).json({ error: "PAYOUT failed", detail: String(e) });
  }
});
...
function findFreePort(startPort, cb) {
  ...
  server.once("error", (err) => {
    if (err.code === "EADDRINUSE") findFreePort(startPort + 1, cb);
    else cb(err);
  });
